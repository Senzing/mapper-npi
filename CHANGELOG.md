...
- Currently-deactivated NPIs (a deactivation date and no reactivation date) are now their own data
  source, `NPI_DEACTIVE`, written to a fifth output file `NPI_DEACTIVE_<period>.json`, so an
  entity that touches a retired NPI is directly queryable. They keep `NPI_NUMBER`, `REF_NPI_ID`
  and `REL_ANCHOR` and carry no `RECORD_TYPE` -- except the 45,630 (of 355,329) that have an
  organization row in `othername_pfile`, which are typed `ORGANIZATION` from that name (Entity
  Type Code is blank on every deactivated row, and no Entity Type 1 NPI ever appears in that
  file). An NPI that was deactivated and later reactivated is active again and stays in
  `NPI-PROVIDERS`.
